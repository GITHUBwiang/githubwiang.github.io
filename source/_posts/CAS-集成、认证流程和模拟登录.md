...
[xianglin2020/cas-demo](https://github.com/xianglin2020/cas-demo)
...
1. 从 cas-overlay-template 中获取 5.3 分支的项目文件，如图
   ...
   ![克隆代码](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/105103.png)
...
2. CAS 默认使用 https，需要先生成证书
   ...
   2. 也可以手动生成，如图
      ...
      ![生成证书](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/105118.png)

      ![导出证书](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/105603.png)
...
4. 使用 WAR 方式启动项目，访问认证页面 `https://cas.server.com:8443/cas/login`，如图
   ...
   ![CAS登录页面](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/113727.png)
...
3. 启动客户端，如图
   ...
   ![客户端集成 CAS](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/174623.png)

   ![重定向到客户端](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/174635.png)
...
4. 启动客户端，如图
   ...
   ![重定向到客户端](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/191228.png)

## 认证流程

![CAS架构](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/191732.png)
...
4. 序号 957，浏览器向 `https://localhost:9443/sample/` 发起请求，携带 CAS 服务器签发的 `ST`，`localhost:9443` 在过滤器中获取到 ticket 值，通过 HTTP 请求方式调用 CAS 服务验证该 ticket 是否有效，过滤器全称 `org.jasig.cas.client.validation.Cas30ProxyReceivingTicketValidationFilter` ，验证方法在 `org.jasig.cas.client.validation.AbstractUrlBasedTicketValidator#validate`， 如图。
   ...
   ![image-20210707091626753](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/091627.png)

   此时，CAS 服务器日志如下图。

   ![CAS 认证--ST 验证](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/091059.png)
...
5. 序号 958，`ST` 验证通过后再次重定向是在 `Cas30ProxyReceivingTicketValidationFilter` 过滤器中使用了 `redirectAfterValidation` 参数，将其配置成 `false` 即可在验证用户后返回正常页面，如图。
   ...
   ![CAS 认证--重定向](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/092652.png)
...
HttpClient 在 SSL 握手时验证对方主机名称，防止链接被重定向到其他地址上去，若服务器证书上的 HostName 和实际的 URL 不匹配时，会抛出如下异常，参考 [java - javax.net.ssl.SSLPeerUnverifiedException: Host name does not match the certificate subject provided by the peer - Stack Overflow](https://stackoverflow.com/questions/34655031/javax-net-ssl-sslpeerunverifiedexception-host-name-does-not-match-the-certifica)。
...
![HttpClient 获取 Cookie](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/140952.png)

HttpClient 自动重定向默认不支持 POST 请求，这也是有“3、重定向到客户端”这一步的原因，HttpClient 使用 `RedirectStrategy` 来完成重定向的处理，默认实现类 `DefaultRedirectStrategy` 中只允许 `GET`、`HEAD`方法的重定向，如果需要支持 `POST` 可以使用 `LaxRedirectStrategy`，如下。
...
正常情况下 Fillder 是无法抓取 HttpClient 请求的，可以通过设置代理的方式，如下。
...
在右侧工具栏找到 Code 标签，Code snippet 中选择“Java - OkHttp”，即可复用代码，如图。
...
![CAS模拟登录、上传文件](https://cdn.jsdelivr.net/gh/xianglin2020/gallery@master/2021-07/164506.png)
